...
if __name__=='__main__':
    ...
    max_count_covid=0
    max_count_vaccine=0
    consumer = KafkaConsumer(bootstrap_servers=['128.110.154.221:9092','128.110.154.213:9092','128.110.154.204:9092'], auto_offset_reset='earliest',
                             consumer_timeout_ms=1000, group_id='my_grp',value_deserializer=lambda x: json.loads(x))
    consumer.subscribe(topics=['covid-tweets-cluster','vaccine-tweets-cluster'],pattern=None,listener=None)
            ...
            for msg1 in msg[1]:
                ...
                if pyobj1.get('matching_rules')[0].get("tag") =="covid123":
                    ...
                    if N_covid<0:
                        ...
                        if Max_ID_covid!=pyobj1.get('ID'):
                            print('anamoly in sequence ID')
                        Max_ID_covid=pyobj1.get('ID')
                        if max_count_covid <= pyobj1.get('data').get('public_metrics').get('retweet_count'):
                            ...
                            Max_retweet_obj_covid="ID: " + str(pyobj1.get('ID')) + ' Tweet with retweet count ' + str(
                                pyobj1.get('data').get('public_metrics').get(
                                    'retweet_count')) + ' is received from the topic:covid-tweets-cluster\n' + pyobj1.get('data').get(
                                'text') + '\n'
                    ...
                    if N_vaccine<0:
                        ...
                        if Max_ID_vaccine!=pyobj1.get('ID')-1:
                            print('anamoly in sequence ID')
                        Max_ID_vaccine=pyobj1.get('ID')
                        if max_count_vaccine <= pyobj1.get('data').get('public_metrics').get('retweet_count'):
                            ...
                            Max_retweet_obj_vaccine="ID: " + str(pyobj1.get('ID')) + ' Tweet with retweet count ' + str(
                                pyobj1.get('data').get('public_metrics').get(
                                    'retweet_count')) + ' is received from the topic:vaccine-tweets-cluster\n' + pyobj1.get('data').get(
                                'text') + '\n'
